    ...
    def _generate_probe(self, critique: str) -> str:
        if not self.P:
            prompt = (
                f"Community: {self.community}\n"
                f"Content: {self.M}\n\n"
                f"Generate an opening question suited to {self.community} to understand the poster's motivation."
            )
        else:
            prompt = (
                f"Community: {self.community}\n"
                f"Probe conversation:\n{self._fmt_feedback()}\n"
                f"Content: {self.M}\n\n"
                f"Generate a follow-up question suited to {self.community} to further uncover the poster's intent."
            )
